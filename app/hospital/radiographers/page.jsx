...
const Radiographer = () => {
  ...
  const fetchRadiographers = async () => {
    try {
      ...
      const response = await fetch("http://localhost:8080/radiographer", {
        method: "GET",
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const token = Cookies.get("token");
      const response = await fetch(`http://localhost:8080/radiographer/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      if (isEdit) {
        // Update Radiographer
        const response = await fetch(`http://localhost:8080/radiographer/${currentRadiographer.id}`, {
          method: "PUT",
          headers: {
        ...
        {selectedRadiographer && (
          <div>
            <Row gutter={16}>
              <Col span={8}>
                <Image
                  src={`http://localhost:8080/${selectedRadiographer.profileUrl}`}
                  alt="Profile"
